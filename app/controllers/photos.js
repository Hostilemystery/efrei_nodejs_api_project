...
const Photos = class Photos {
  ...
  /**
   * Create photo (Protected route)
   */
  createPhoto() {
    this.app.post('/albums/:albumId/photos', this.authenticateToken, async (req, res) => {
      const photo = new this.PhotoModel({
        ...req.body,
        album: req.params.albumId
      });
  ...
  /**
   * Update photo (Protected route)
   */
  updatePhoto() {
    this.app.put('/albums/:albumId/photos/:photoId', this.authenticateToken, async (req, res) => {
      try {
        const updatedPhoto = await this.PhotoModel.findOneAndUpdate(
          { _id: req.params.photoId, album: req.params.albumId },
          req.body,
   ...
   */
  deletePhoto() {
    this.app.delete('/albums/:albumId/photos/:photoId', this.authenticateToken, async (req, res) => {
      try {
        const photo = await this.PhotoModel.findOneAndDelete({ _id: req.params.photoId, album: req.params.albumId });
        if (!photo) return res.status(404).json({ message: 'Photo Not Found!' });
        await this.AlbumModel.findByIdAndUpdate(req.params.albumId, { $pull: { photos: photo._id } });
   ...
   */
  run() {
    this.getAllPhotos();    // Public
    this.createPhoto();      // Protected
    this.updatePhoto();      // Protected
    this.deletePhoto();      // Protected
  }
};
